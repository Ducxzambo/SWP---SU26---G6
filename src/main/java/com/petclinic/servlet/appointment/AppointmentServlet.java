...
public class AppointmentServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest req, HttpServletResponse resp)
        ...
        try {
            switch (req.getServletPath()) {
                case "/appointments":
                    handleList(req, resp, customer);
                    break;
                case "/appointments/detail":
                    handleDetail(req, resp, customer);
                    break;
                case "/appointments/reschedule":
                    handleRescheduleGet(req, resp, customer);
                    break;
                case "/appointments/pay":
                    handlePay(req, resp, customer);
                    break;
                default:
    ...
    protected void doPost(HttpServletRequest req, HttpServletResponse resp)
        ...
        try {
            switch (req.getServletPath()) {
                case "/appointments/reschedule":
                    handleReschedulePost(req, resp, customer);
                    break;
                case "/appointments/cancel":
                    handleCancel(req, resp, customer);
                    break;
                default:
                    resp.sendRedirect(req.getContextPath() + "/appointments");
            }
    ...
    private void handleList(HttpServletRequest req, HttpServletResponse resp, Customer customer)
        ...
        List<ServiceCategory> navCats = serviceDAO.findAllCategoriesWithServices();
        if (navCats == null) navCats = Collections.emptyList();
        req.setAttribute("navCategories", navCats);
    ...
    private void handleReschedulePost(HttpServletRequest req, HttpServletResponse resp, Customer customer)
        ...
        String slotKey = req.getParameter("slotKey"); // "yyyy-MM-dd|HH:mm"
        ...
        LocalTime end = start.plusMinutes(25);
        ...
        for (Map.Entry<LocalDate, List<TimeSlot>> e : slots.entrySet()) {
            ...
            if (slotList != null) {
                for (TimeSlot ts : slotList) {
                    ...
                    if (!fs) sb.append(",");
                    fs = false;
                    sb.append("{\"key\":\"").append(esc(ts.getSlotKey())).append("\"")
