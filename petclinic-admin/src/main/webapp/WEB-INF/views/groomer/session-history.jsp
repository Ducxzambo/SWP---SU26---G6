...
<div class="layout">
  <aside class="sidebar">
    ...
    <nav>
      <a href="${pageContext.request.contextPath}/groomer/session" class="nav-item">Hàng chờ</a>
      <a href="${pageContext.request.contextPath}/groomer/session?action=history" class="nav-item active">Lịch sử</a>
    </nav>
  ...
  <main class="main-content">
    <div class="page-header">
      <h1>Lịch Sử</h1>
      <p class="page-sub">Các ca bạn đã lưu kết quả grooming xong (lịch hẹn có thể vẫn chờ nhân viên khác hoàn tất)</p>
    </div>
    ...
    <div class="toolbar">
      ...
      <c:if test="${!isToday}">
        <a href="${pageContext.request.contextPath}/groomer/session?action=history" class="btn btn-outline btn-sm" style="margin-bottom:1px;">
          Về hôm nay
        </a>
      </c:if>
    </div>
    ...
    <div class="shift-tabs">
      <a href="${pageContext.request.contextPath}/groomer/session?action=history&date=${filterDate}"
         class="shift-tab ${empty shiftFilter ? 'active' : ''}">Tất cả ca</a>
      <a href="${pageContext.request.contextPath}/groomer/session?action=history&date=${filterDate}&shift=1"
         class="shift-tab ${shiftFilter == '1' ? 'active' : ''}">Ca 1 <span class="count">08:00–10:00</span></a>
      <a href="${pageContext.request.contextPath}/groomer/session?action=history&date=${filterDate}&shift=2"
         class="shift-tab ${shiftFilter == '2' ? 'active' : ''}">Ca 2 <span class="count">10:00–12:00</span></a>
      <a href="${pageContext.request.contextPath}/groomer/session?action=history&date=${filterDate}&shift=3"
         class="shift-tab ${shiftFilter == '3' ? 'active' : ''}">Ca 3 <span class="count">13:30–15:30</span></a>
      <a href="${pageContext.request.contextPath}/groomer/session?action=history&date=${filterDate}&shift=4"
      ...
      <c:choose>
        <c:when test="${empty completed}">
          <div class="empty-state">
            ...
            <p>Chưa có phiên grooming nào bạn đã hoàn thành trong ngày này.</p>
          </div>
        </c:when>
        <c:otherwise>
