...
public class CheckInServlet extends HttpServlet {
            ...
            try {
                ...
                session.setAttribute("flashWarning",
                        "Walk-in thành công (Lịch khám #" + apptID
                                + ") nhưng không tạo được hóa đơn tự động. Vui lòng liên hệ quản trị viên.");
                resp.sendRedirect(req.getContextPath() + "/receptionist/checkin");
                return;
            }
            resp.sendRedirect(req.getContextPath() + "/receptionist/invoice?invoiceId=" + invoiceId + "&from=checkin");
    ...
    private void loadFormOptions(HttpServletRequest req) {
        try {
            ...
            List<Service> services = examinationService.getAllActiveServices();
