package com.petclinic.admin.servlet.groomer;
            ...
            switch (result) {
                case SUCCESS -> {
                    req.getSession().setAttribute("flashSuccess",
                            "Phiên grooming đã hoàn thành! Hóa đơn đã được cập nhật.");
                    invoiceSyncService.syncAfterGrooming(appointmentID, groomer.getStaffID());
                    resp.sendRedirect(req.getContextPath() + "/groomer/session");
                }
