...
public class EmailService {
    ...
    public void onRefundProcessed(Customer customer, Refund refund, Appointment appt) {
        sendAsync(() -> {
            ...
            String body = buildRefundProcessedHtml(customer, refund, appt);
            boolean ok = send(customer.getEmail(), subject, body);
            if (ok) {
                LOG.info("Refund-processed email sent to " + customer.getEmail()
                        + " for refund #" + refund.getRefundID());
            } else {
                LOG.warning("Refund-processed email NOT sent to " + customer.getEmail()
                        + " for refund #" + refund.getRefundID());
            }
        });
    }
    ...
    public void onRefundRejected(Customer customer, Refund refund, Appointment appt) {
        sendAsync(() -> {
            ...
            String body = buildRefundRejectedHtml(customer, refund, appt);
            boolean ok = send(customer.getEmail(), subject, body);
            if (ok) {
                LOG.info("Refund-rejected email sent to " + customer.getEmail()
                        + " for refund #" + refund.getRefundID());
            } else {
                LOG.warning("Refund-rejected email NOT sent to " + customer.getEmail()
                        + " for refund #" + refund.getRefundID());
            }
        });
    }
    ...
    private String buildRefundProcessedHtml(Customer customer, Refund refund, Appointment appt) {
        return "<!DOCTYPE html><html><head><meta charset='UTF-8'></head><body>"
                + "<div style='font-family:sans-serif;max-width:560px;margin:auto;padding:24px;'>"
                + "<h2 style='color:#0f3d24;'>🐾 PetClinic – Đã hoàn tiền</h2>"
                + "<p>Xin chào <strong>" + esc(customer.getFullName()) + "</strong>,</p>"
                + "<p>Yêu cầu hoàn tiền của bạn đã được xử lý thành công. Số tiền đã được "
                +   "chuyển khoản về tài khoản bạn cung cấp.</p>"
                + "<div style='background:#f0faf4;border:1px solid #d6f0e2;border-radius:10px;"
                +      "padding:18px 20px;margin:20px 0;'>"
                + "<table style='width:100%;border-collapse:collapse;font-size:14px;'>"
                + row("Dịch vụ",       appt != null ? esc(appt.getServiceName()) : "-")
                + row("Thú cưng",      petOrPlaceholder(appt))
                + row("Số tiền hoàn",  "<span style='color:#155724;font-weight:600;'>"
                + fmt(refund.getPaidAmount()) + "₫</span>")
                + row("Ngân hàng nhận", esc(refund.getBankCode()) + " – " + esc(refund.getAccountNumber()))
                + row("Thời gian",     refund.getFormattedRefundedAt())
                + "</table></div>"
                + "<p style='font-size:13px;color:#8c8680;'>Nếu bạn không nhận được tiền trong vòng "
                +   "1-2 ngày làm việc, vui lòng liên hệ chúng tôi.</p>"
                + "<p style='font-size:13px;color:#8c8680;'>Hotline: <strong>(028) 123 456 789</strong></p>"
                + "<hr style='border:none;border-top:1px solid #d8d4cc;margin:20px 0;'>"
                + "<p style='font-size:12px;color:#b8b4ae;text-align:center;'>© 2025 PetClinic. 123 Đường ABC, Q.1, TP.HCM</p>"
                + "</div></body></html>";
    ...
    private String buildRefundRejectedHtml(Customer customer, Refund refund, Appointment appt) {
        return "<!DOCTYPE html><html><head><meta charset='UTF-8'></head><body>"
                + "<div style='font-family:sans-serif;max-width:560px;margin:auto;padding:24px;'>"
                + "<h2 style='color:#856404;'>PetClinic – Yêu cầu hoàn tiền bị từ chối</h2>"
                + "<p>Xin chào <strong>" + esc(customer.getFullName()) + "</strong>,</p>"
                + "<p>Rất tiếc, yêu cầu hoàn tiền của bạn <strong>không được chấp nhận</strong>.</p>"
                + "<div style='background:#fff3cd;border:1px solid #ffc107;border-radius:10px;"
                +      "padding:18px 20px;margin:20px 0;'>"
                + "<table style='width:100%;border-collapse:collapse;font-size:14px;'>"
                + row("Dịch vụ",  appt != null ? esc(appt.getServiceName()) : "-")
                + row("Thú cưng", petOrPlaceholder(appt))
                + row("Số tiền yêu cầu", fmt(refund.getPaidAmount()) + "₫")
                + row("Lý do từ chối", "<span style='color:#856404;'>"
                + esc(refund.getRejectReason()) + "</span>")
                + "</table></div>"
                + "<p style='font-size:13px;color:#8c8680;'>Nếu có thắc mắc, vui lòng liên hệ chúng tôi.</p>"
                + "<p style='font-size:13px;color:#8c8680;'>Hotline: <strong>(028) 123 456 789</strong></p>"
                + "<hr style='border:none;border-top:1px solid #d8d4cc;margin:20px 0;'>"
                + "<p style='font-size:12px;color:#b8b4ae;text-align:center;'>© 2025 PetClinic. 123 Đường ABC, Q.1, TP.HCM</p>"
                + "</div></body></html>";
