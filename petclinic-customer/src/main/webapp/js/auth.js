...
// Render every auth icon as inline SVG so the UI does not depend on emoji glyphs.
function initInlineAuthIcons() {
  const icons = {
    ...
    phone: '<svg viewBox="0 0 24 24" aria-hidden="true"><path d="M22 16.9v3a2 2 0 0 1-2.2 2A19.8 19.8 0 0 1 2.1 4.2 2 2 0 0 1 4.1 2h3a2 2 0 0 1 2 1.7c.1 1 .4 1.9.7 2.8a2 2 0 0 1-.5 2.1L8 9.9a16 16 0 0 0 6.1 6.1l1.3-1.3a2 2 0 0 1 2.1-.5c.9.3 1.8.6 2.8.7A2 2 0 0 1 22 16.9z"/></svg>',
  ...
  document.querySelectorAll('.input-icon').forEach(el => {
    ...
    el.innerHTML = type === 'password' ? icons.lock : type === 'email' || name.includes('email') ? icons.mail : type === 'tel' || name.includes('phone') ? icons.phone : icons.user;
  });
  document.querySelectorAll('.toggle-pwd').forEach(el => {
    el.innerHTML = icons.eye;
    el.setAttribute('aria-label', 'Hiện mật khẩu');
  });
}
